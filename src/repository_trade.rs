...
struct RepositoryTrade {
    pool: PgPool,
}

impl RepositoryTrade {
    ...
    pub fn read_by_id(&self, id: Decimal) -> eyre::Result<Option<Trade>> {
        let future = sqlx::query_as!(Trade, "SELECT * FROM trade WHERE id = $1", id)
            .fetch_optional(&self.pool);
        Ok(async_std::task::block_on(future)?)
    }

    /// Insert trades
    pub fn insert_trades(&self, trades: &mut [Trade]) -> eyre::Result<()> {
    ...
    pub fn insert_trade(&self, trade: &Trade) -> eyre::Result<Decimal> {
